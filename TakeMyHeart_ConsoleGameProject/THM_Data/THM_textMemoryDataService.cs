    ...
    internal class THM_textMemoryDataService : IThm_DataService
    {
        ...
        private void GetStoryFromFile()
        {
            ...
            for (int i = 0; i < storyLines.Length; i++)
            {
                ...
                //if (string.IsNullOrWhiteSpace(TxtstoryDialougeA[i]))
                //    continue;
                //storyDialougeA[i] = TxtstoryDialougeA[i];
                //if (string.IsNullOrWhiteSpace(TxtstoryDialougeB[i]))
                //    continue;
                //storyDialougeB[i] = TxtstoryDialougeB[i];

                //if (string.IsNullOrWhiteSpace(choicesA[i]))
                //    continue;
                //choicesA[i] = TxtchoicesA[i];
                //if (string.IsNullOrWhiteSpace(choicesB[i]))
                //    continue;
                //choicesB[i] = TxtchoicesB[i];
            }
